 ...
 *
 * <p>Basic calculations with rational numbers (+ - * /) have no loss of precision. This allows to
 * use {@link BigRational} as a replacement for {@link BigDecimal} if absolute accuracy is desired.
 *
 * <p><a href="http://en.wikipedia.org/wiki/Rational_number">Wikipedia: Rational number</a>
 *
 * <p>The values are internally stored as {@link BigDecimal} (for performance optimizations) but
 * represented as {@link BigInteger} (for mathematical correctness) when accessed with {@link
 * #getNumeratorBigInteger()} and {@link #getDenominatorBigInteger()}.
 *
 * <p>The following basic calculations have no loss of precision:
 *
 * <ul>
 *   <li>{@link #add(BigRational)}
 *   <li>{@link #subtract(BigRational)}
 *   <li>{@link #multiply(BigRational)}
 *   <li>{@link #divide(BigRational)}
 *   <li>{@link #pow(int)}
 * </ul>
 *
 * <p>The following calculations are special cases of the ones listed above and have no loss of
 * precision:
 *
 * <ul>
 *   <li>{@link #negate()}
 ...
 * </ul>
 *
 * <p>Any {@link BigRational} value can be converted into an arbitrary {@link #withPrecision(int)
 * precision} (number of significant digits) or {@link #withScale(int) scale} (number of digits
...
public class BigRational implements Comparable<BigRational> {
   ...
   *
   * <p>Guaranteed to not be 0.
   *
   * <p>Guaranteed to be positive.
   *
   ...
   *
   * <ul>
   *   <li><code>BigRational.valueOf(3.5).integerPart()</code> returns <code>BigRational.valueOf(3)
   *       </code>
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li><code>BigRational.valueOf(3.5).integerPart()</code> returns <code>
   *       BigRational.valueOf(0.5)</code>
   * </ul>
   *
   ...
   *
   * <p>The result has no loss of precision.
   *
   * <p>Examples:
   *
   * <ul>
   *   <li><code>BigRational.valueOf(3.5).negate()</code> returns <code>BigRational.valueOf(-3.5)
   *       </code>
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li><code>BigRational.valueOf(0.5).reciprocal()</code> returns <code>BigRational.valueOf(2)
   *       </code>
   *   <li><code>BigRational.valueOf(-2).reciprocal()</code> returns <code>BigRational.valueOf(-0.5)
   *       </code>
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li><code>BigRational.valueOf(-2).abs()</code> returns <code>BigRational.valueOf(2)</code>
   *   <li><code>BigRational.valueOf(2).abs()</code> returns <code>BigRational.valueOf(2)</code>
   * </ul>
   *
   ...
   *
   * <p>This is functionally identical to <code>this.add(BigRational.ONE)</code> but slightly
   * faster.
   *
   * <p>The result has no loss of precision.
   *
   ...
   *
   * <p>This is functionally identical to <code>this.subtract(BigRational.ONE)</code> but slightly
   * faster.
   *
   * <p>The result has no loss of precision.
   *
   ...
   * Calculates the addition (+) of this rational number and the specified argument.
   *
   * <p>This is functionally identical to <code>this.add(BigRational.valueOf(value))</code> but
   * slightly faster.
   *
   * <p>The result has no loss of precision.
   *
  ...
  public BigRational subtract(BigRational value) {
    ...
    BigDecimal n =
        numerator.multiply(value.denominator).subtract(value.numerator.multiply(denominator));
    BigDecimal d = denominator.multiply(value.denominator);
    return of(n, d);
  }
   ...
   * Calculates the subtraction (-) of this rational number and the specified argument.
   *
   * <p>This is functionally identical to <code>this.subtract(BigRational.valueOf(value))</code> but
   * slightly faster.
   *
   * <p>The result has no loss of precision.
   *
   ...
   * Calculates the multiplication (*) of this rational number and the specified argument.
   *
   * <p>This is functionally identical to <code>this.multiply(BigRational.valueOf(value))</code> but
   * slightly faster.
   *
   * <p>The result has no loss of precision.
   *
   ...
   * Calculates the division (/) of this rational number and the specified argument.
   *
   * <p>This is functionally identical to <code>this.divide(BigRational.valueOf(value))</code> but
   * slightly faster.
   *
   * <p>The result has no loss of precision.
   *
   ...
   *
   * @return <code>true</code> if this rational number is zero (0), <code>false</code> if it is not
   *     zero
   */
   ...
   *
   * @return <code>true</code> if this rational number is an integer number, <code>false</code> if
   *     it has a fraction part
   */
   ...
   * Returns whether this rational number is an integer number without fraction part.
   *
   * <p>Will return <code>false</code> if this number is not reduced to the integer representation
   * yet (e.g. 4/4 or 4/2)
   *
   * @return <code>true</code> if this rational number is an integer number, <code>false</code> if
   *     it has a fraction part
   ...
   * @return the minimum rational number, either <code>this</code> or the argument <code>value
   *     </code>
   */
  ...
  /**
   * Returns a rational number with approximatively <code>this</code> value and the specified
   * precision.
   *
   * @param precision the precision (number of significant digits) of the calculated result, or 0
  ...
  /**
   * Returns this rational number as a {@link BigDecimal} with the precision specified by the {@link
   * MathContext}.
   *
   ...
   *
   * <p>The resulting string is a valid input of the {@link #valueOf(String)} method.
   *
   * <p>Examples:
   *
   * <ul>
   *   <li><code>BigRational.valueOf(0.5).toRationalString()</code> returns <code>"1/2"</code>
   *   <li><code>BigRational.valueOf(2).toRationalString()</code> returns <code>"2"</code>
   *   <li><code>BigRational.valueOf(4, 4).toRationalString()</code> returns <code>"4/4"</code> (not
   *       reduced)
   * </ul>
   *
   * @return the rational number string representation in the form "numerator/denominator", or "0"
   *     if the rational number is 0.
   * @see #valueOf(String)
   * @see #valueOf(int, int)
   */
  ...
  /**
   * Returns the string representation of this rational number as integer and fraction parts in the
   * form "integerPart fractionNominator/fractionDenominator".
   *
   * <p>The integer part is omitted if it is 0 (when this absolute rational number is smaller than
   * 1).
   *
   * <p>The fraction part is omitted it it is 0 (when this rational number is an integer).
   *
   * <p>If this rational number is 0, then "0" is returned.
   *
   * <p>Example: <code>BigRational.valueOf(3.5).toIntegerRationalString()</code> returns <code>
   * "3 1/2"</code>.
   *
   ...
   *
   * <p>Useful to create numbers like 3 1/2 (= three and a half = 3.5) by calling <code>
   * BigRational.valueOf(3, 1, 2)</code>.
   *
   * <p>To create a negative rational only the integer part argument is allowed to be negative: to
   * create -3 1/2 (= minus three and a half = -3.5) call <code>BigRational.valueOf(-3, 1, 2)</code>
   * .
   *
   ...
   * @return the rational number
   * @throws ArithmeticException if the fraction part denominator is 0 (division by zero), or if the
   *     fraction part numerator or denominator is negative
   */
  public static BigRational valueOf(int integer, int fractionNumerator, int fractionDenominator) {
    ...
    return integerPart.isPositive()
        ? integerPart.add(fractionPart)
        : integerPart.subtract(fractionPart);
  }
  ...
  public static BigRational valueOf(BigDecimal value) {
    ...
    if (scale == 0) {
      ...
      BigDecimal n =
          new BigDecimal(value.unscaledValue())
              .multiply(BigDecimal.ONE.movePointLeft(value.scale()));
   ...
   *
   * <ul>
   *   <li>Output of {@link BigRational#toString()} : "integerPart.fractionPart"
   *   <li>Output of {@link BigRational#toRationalString()} : "numerator/denominator"
   *   <li>Output of <code>toString()</code> of {@link BigDecimal}, {@link BigInteger}, {@link
   *       Integer}, ...
   *   <li>Output of <code>toString()</code> of {@link Double}, {@link Float} - except "Infinity",
   *       "-Infinity" and "NaN"
   * </ul>
   *
  ...
  public static BigRational valueOf(
    ...
    if (fractionRepeatPart != null && fractionRepeatPart.length() > 0) {
      BigInteger lotsOfNines =
          BigInteger.TEN.pow(fractionRepeatPart.length()).subtract(BigInteger.ONE);
   ...
   *
   * <p>This function calculates the <strong>first Bernoulli numbers</strong> and therefore <code>
   * bernoulli(1)</code> returns -0.5
   *
   * <p>Note that <code>bernoulli(x)</code> for all odd x &gt; 1 returns 0
   *
   * <p>See: <a href="https://en.wikipedia.org/wiki/Bernoulli_number">Wikipedia: Bernoulli
   * number</a>
   *
